...
let currentFamilyRooms = []; 
let pendingSelectedDefaults = new Set(); 
let pendingTags = []; 
...
window.announce = (msg, type = 'normal') => {
    ...
    setTimeout(() => {
        ...
        if (msg.includes("成功") || msg.includes("已添加") || msg.includes("已删除") || msg.includes("已更新") || msg.includes("自动填入") || msg.includes("已切换")) {
            playSound('success');
        } else if (msg.includes("失败") || msg.includes("错误") || msg.includes("不足") || msg.includes("未找到") || msg.includes("请先")) {
            playSound('error');
        }
    }, 50);
...
function setupDataListener(uid) {
    ...
    const q = query(itemsRef, where("uid", "==", uid), where("familyId", "==", currentAppFamily.id));
    ...
    unsubscribeItems = onSnapshot(q, snap => {
        ...
        snap.docChanges().forEach(change => {
            ...
            if(data.unit) learnNewUnit(data.unit);
...
function loadUserProfile(user) {
    ...
    unsubscribeProfile = onSnapshot(doc(db, "profiles", user.uid), (docSnap) => {
        ...
        if (btnAccount) {
            ...
            btnAccount.setAttribute('aria-label', `当前账号：${displayNick}，${email}，家庭：${familyName}，点击展开菜单`);
        }
...
function toggleFamilyForm(show, mode = 'new') {
    ...
    if (show) {
        ...
        box.classList.add('hidden');
        document.getElementById('btn-fam-new').focus(); 
    }
}
...
function renderDeleteRoomList() {
    ...
    currentFamilyRooms.forEach((room, index) => {
        ...
        div.innerHTML = `
            <input type="checkbox" class="peer absolute inset-0 w-full h-full opacity-0 z-10 cursor-pointer" tabindex="${index === 0 ? '0' : '-1'}" aria-label="${room}">
            <div class="flex items-center justify-between p-4 rounded-lg border-2 border-gray-200 bg-white transition-all peer-focus:ring-4 peer-focus:ring-red-200 peer-focus:border-red-600 peer-checked:bg-red-50 peer-checked:border-red-600" aria-hidden="true">
                <span class="text-xl font-bold text-gray-800">${room}</span>
                <div class="w-8 h-8 rounded-full border-2 border-gray-300 flex items-center justify-center bg-white peer-checked:border-red-600 peer-checked:bg-red-600">
        ...
        input.addEventListener('change', () => {
            if(input.checked) pendingDeleteRooms.add(room); else pendingDeleteRooms.delete(room);
        });
        input.addEventListener('keydown', (e) => {
            if(e.key === 'ArrowDown' || e.key === 'ArrowUp') {
                ...
                let nextIndex;
                if (e.key === 'ArrowDown') nextIndex = (currentIndex + 1) % allInputs.length;
                else nextIndex = (currentIndex - 1 + allInputs.length) % allInputs.length;
...
safeListen('btn-confirm-del-rooms', 'click', () => {
    if(pendingDeleteRooms.size === 0) { announce("未勾选任何房间"); return; }
...
function renderPresetKeyboardList() {
    ...
    availableDefaults.forEach((room, index) => {
        ...
        div.innerHTML = `
            <span class="text-xl font-bold text-gray-800">${room}</span>
            <div class="w-8 h-8 rounded-full border-2 border-gray-300 flex items-center justify-center status-icon"><span class="hidden text-white font-bold">✓</span></div>
        `;
...
function toggleRoomSelection(el, roomName) {
    ...
    if (pendingSelectedDefaults.has(roomName)) {
        ...
        el.classList.remove('bg-blue-50', 'border-blue-500'); el.classList.add('border-gray-200');
        iconBox.classList.remove('bg-blue-600', 'border-blue-600'); iconBox.classList.add('border-gray-300');
        checkMark.classList.add('hidden');
        announce(`已取消 ${roomName}`);
    } else {
        pendingSelectedDefaults.add(roomName);
        el.setAttribute('aria-checked', 'true');
        el.classList.add('bg-blue-50', 'border-blue-500'); el.classList.remove('border-gray-200');
        iconBox.classList.add('bg-blue-600', 'border-blue-600'); iconBox.classList.remove('border-gray-300');
...
function handleRoomListKeydown(e, currentEl, roomName) {
    if (e.key === ' ' || e.key === 'Enter') {
        e.preventDefault(); toggleRoomSelection(currentEl, roomName);
    } else if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
        e.preventDefault();
        ...
        let nextIndex;
        if (e.key === 'ArrowDown') nextIndex = (currentIndex + 1) % allItems.length;
        else nextIndex = (currentIndex - 1 + allItems.length) % allItems.length;
...
safeListen('btn-confirm-add-rooms', 'click', async () => {
     ...
     if(customName) {
         if(!currentFamilyRooms.includes(customName)) pendingSelectedDefaults.add(customName);
     }
     if (pendingSelectedDefaults.size === 0) { announce("请选择或输入至少一个新房间"); return; }
     pendingSelectedDefaults.forEach(r => { if (!currentFamilyRooms.includes(r)) currentFamilyRooms.push(r); });
     await saveRoomsToFirestore();
...
safeListen('room-family-select', 'change', async (e) => {
     ...
     try {
        const famDoc = await getDoc(doc(db, "families", currentFamilyId));
        if (famDoc.exists()) currentFamilyRooms = famDoc.data().rooms || [];
        else currentFamilyRooms = [];
...
safeListen('btn-add-qty-trigger', 'click', () => {
    openQtyPicker("初始数量", (val) => { pendingAddQty = val; updateAddQtyDisplay(); });
});
...
safeListen('form-add', 'submit', async (e) => {
    ...
    try {
        await addDoc(itemsRef, {
            ...
            familyId: currentAppFamily.id, // 核心：写入家庭ID
            uid: auth.currentUser.uid,
            updatedAt: serverTimestamp()
        });
        announce("添加成功");
